    ...
    for output in vc.outputs:
        ...
        if only_areas:
            for a in area_lineage:
                if a in only_areas:
                    _LOGGER.debug("maybe including %s because of only_areas = %s",
            ...
            if keep and exclude_areas:
                for a in area_lineage:
                    if a in exclude_areas:
                        _LOGGER.debug("but %s is in exclude_areas, so skipping", a)
                        keep = False
                        break
            ...
            for a in area_lineage:
                if a in exclude_areas:
                    _LOGGER.debug("discarding %s because of exclude_areas = %s",
